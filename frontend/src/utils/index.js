...
// Custom Raw Tool with Edit/Preview Mode
class CustomRawTool extends RawTool {
	...
	render() {
		// VIEW MODE: Render HTML directly for students
		if (this.readOnly) {
		...
		// EDIT MODE: Creative UI with Toggle
		this.wrapper = document.createElement('div')
		this.wrapper.style.position = 'relative'
		this.wrapper.style.marginBottom = '10px'
        this.wrapper.style.marginTop = '20px'

		// 1. Text Area (Code Input)
		const textarea = document.createElement('textarea')
		textarea.value = this.data.html || ''
		textarea.placeholder = 'Paste HTML or Iframe code here...'
		textarea.classList.add('ce-rawtool__textarea', 'cdx-input')
		textarea.style.width = '100%'
		textarea.style.minHeight = '100px'
		textarea.style.fontFamily = 'monospace'
		textarea.style.fontSize = '13px'
		textarea.style.padding = '10px'
		textarea.style.backgroundColor = '#1e1e1e' // Dark code theme
		...
		// 2. Preview Container (Hidden initially)
		...
		toggleBtn.onclick = () => {
			if (preview.style.display === 'none') {
				// Show Preview
				preview.innerHTML = this.data.html || '<span style="color:gray">Empty</span>'
	...
	save(blockContent) {
		if (this.readOnly) return this.data
		const textarea = blockContent.querySelector('textarea')
		return {
			html: textarea ? textarea.value : ''
		}
	}
...
export function getEditorTools() {
	return {
		...
		raw: {
			...
			inlineToolbar: true,
			config: {
				placeholder: 'Paste HTML/Iframe code here...',
			},
		},
		header: {
			class: Header,
